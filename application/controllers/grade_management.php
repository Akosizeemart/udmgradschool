...
class Grade_Management extends CI_Controller {
	...
	function index() {
		...
		$data['page_title'] = 'Universidad de Manila - Grad School:: Grade Management';
	...
	function manage_student(){
		if(!empty($_GET)) {
			...
			$data['page_title'] = 'Universidad de Manila Graduate School :: Grade Management';
			$data['selected']   = 'grade_management';
	...
	function download_student_grade() {
		...
		function download_student_gradelists_form() {
		if(!empty($_POST)) {
			ini_set("memory_limit", "999M");
			set_time_limit(999999999999999999999);
			...
			// echo "</pre>";
			// exit();
			$this->load->view('grade_management/admin/grade_list_excel.php',$data);
		}
	}
